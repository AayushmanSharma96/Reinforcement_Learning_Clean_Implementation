...
ENV_NAME = 'OrbitTransfer-v0'
k = 0
csv_file = 'sac_rendorbitezvous_output_v0_buffer10_6_1M_'+str(k)+'.csv' #csv file to store training progress
exp_name = 'sac_orbit_v0_buffer10_6_1M_'+str(k)
run_name = 'sac'
...
def reward_function(observation, action, next_obs):
    diag_q = 10 * np.array([1e-4, 1, 0.5, 1e-3])
    r = .1*np.array([.1,1])#0.000001*np.array([1,1,1,1000,1000,1000])
    dt = 5
    x_t = np.array([42157,0,3.07,0])
    ...
    cost = diag_q[0]*np.square(observation[0]-x_t[0])+diag_q[1]*np.square(observation[1]-x_t[1])+diag_q[2]*np.square(observation[2]-x_t[2])
    
    ctrl_cost = r[0]*np.square(action[0])+r[1]*np.square(action[1])
...
def make_env(env_id, render_bool):
    ...
    if render_bool:
        ...
        env = gym.make('OrbitTransfer-v0',render_mode = "human")
    else:
        env = gym.make('OrbitTransfer-v0')
