...
ENV_NAME = 'Rendezvous_v0'
k = 2000
csv_file = 'sac_rendezvous_output_v0_buffer10_3_1M_'+str(k)+'.csv' #csv file to store training progress
exp_name = 'sac_rendezvous_v0_buffer10_3_1M_'+str(k)
run_name = 'sac'
u_ilqr = np.load('u_nom_transposed.npy')


class SoftQNetwork(nn.Module):
    def __init__(self, env):
        super().__init__()
        self.fc1 = nn.Linear(np.array(env.observation_space.shape).prod() + np.prod(env.action_space.shape), 600)
...
def reward_function(observation, action, next_obs):
    diag_q = 1e-2*2*np.array([1,1,1,1e5,1e5,1e5,0,0,0,0,0,0,0]) 
    r = 1e-2*2000*np.array([1,1,1])#0.000001*np.array([1,1,1,1000,1000,1000])
...
def make_env(env_id, render_bool):
    ...
    if render_bool:
        ...
        env = gym.make('Rendezvous_v0',render_mode = "human")
    else:
        env = gym.make('Rendezvous_v0')
...
def save_observations_to_numpy(rb, filename="sac_replay_buffer_observations_10_6_", k="1"):
...
if __name__ == "__main__":
    ...
    given_seed = np.random.randint(100)
    buffer_size = int(1e6)
    batch_size = 256
    total_timesteps = 1000000 #default = 1000000
    learning_starts = 50000 #default = 25e3
    episode_length = 1000
    ...
    qf1.apply(initialize_weights)
    qf2.apply(initialize_weights)
    ...
    target_entropy = -np.prod(env.action_space.shape).item()  # Target entropy based on action dimensions
    log_alpha = torch.zeros(1, requires_grad=True, device=device)  # Log of entropy coefficient
    alpha_optimizer = optim.Adam([log_alpha], lr=learning_rate)  # Optimizer for log_alpha
    alpha = log_alpha.exp().item()  # Initialize alpha
    ...
    # Initialize the replay buffer with `u_ilqr` actions#######################
    # obs, _ = env.reset()
    # for i in range(2000):  # Add the first 200 actions
    #     action = u_ilqr[i%1000]
    #     next_obs, reward, done, truncated, info = env.step(action)
    #     print('Observations : ', next_obs)
    #     reward = reward_function(obs, action, next_obs)
    ...
    # print("Replay buffer initialized with the first 200 actions from u_ilqr.")
    ############################################################################
    ...
    for global_step in range(total_timesteps):
        ...
        if global_step < learning_starts:
            if global_step<1000:
                actions = u_ilqr[global_step]+1e-1*np.random.normal(np.shape(u_ilqr[global_step]))
        ...
        if global_step > learning_starts:
            ...
            qf_loss.backward()
            torch.nn.utils.clip_grad_norm_(qf1.parameters(), max_norm=10)  # Clip Q1 gradients
            torch.nn.utils.clip_grad_norm_(qf2.parameters(), max_norm=10)  # Clip Q2 gradients
            q_optimizer.step()
            ...
            if global_step % policy_frequency == 0:
                ...
                for _ in range(policy_frequency):
                    ...
                    # regularization = 0.01 * (pi ** 2).mean()## To delete
                    # actor_loss += regularization## TO DELETE
                    ...
                    # Update log_alpha (adaptive entropy coefficient)
        ...
        if episode_t == episode_length or abs(next_obs[6])<=0:
            print('Observation : ', next_obs)
            print('resetting')
            episode_count += 1
            if episode_count % 20 == 0 and global_step > learning_starts:
    ...
    if save_model:
        ...
        torch.save((actor.state_dict(), qf1.state_dict(), qf2.state_dict(), log_alpha), model_path)
