...
k = 9
csv_file = 'sac_cartpole_output_v4_buffer10_3_1M_'+str(k)+'.csv' #csv file to store training progress
...
def save_observations_to_numpy(rb, filename="1sac_replay_buffer_cartpole_observations_10_3_", k="1"):
...
if __name__ == "__main__":
    ...
    buffer_size = int(1e3)
    batch_size = 256
    total_timesteps = 90000#1000000 #default = 1000000
    ...
    for global_step in range(total_timesteps):
        ...
        if global_step>80000 and global_step<90000:
            if np.shape(rb.observations[:rb.pos])[0]>980:
                save_observations_to_numpy(rb, k="2")
                model_path = f"../runs/{run_name}/{exp_name}_rollouts_inter.pth"
                torch.save((actor.state_dict(), qf1.state_dict(), qf2.state_dict()), model_path)
                print(f"model saved to {model_path}")
            
        if global_step>950000 and global_step<970000:
            save_observations_to_numpy(rb, k="3")
            model_path = f"../runs/{run_name}/{exp_name}_rollouts.pth"
            torch.save((actor.state_dict(), qf1.state_dict(), qf2.state_dict()), model_path)
            print(f"model saved to {model_path}")
