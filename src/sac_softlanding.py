...
ENV_NAME = 'SoftLander-v0'
k = 1
csv_file = 'sac_softland_output_v0_buffer10_6_1M_'+str(k)+'.csv' #csv file to store training progress
exp_name = 'sac_softland_v0_buffer10_6_1M_'+str(k)
run_name = 'sac'
...
def reward_function(observation, action, next_obs):
    diag_q = 0.000001*np.array([0.2*1e3,0.2*1e3,0.2*1e3,0.2*1e3,0.2*1e3,0.2*1e3,0.2*5e8,0.2*5e8,0.2*5e6,0.2*1e6,0.2*1e6,0.2*2e6,0]) 
    r = 0.000001*np.array([1,1,1,1000,1000,1000])
    dt = 0.2
    ...
    cost = diag_q[0]*observation[0]*observation[0]+diag_q[1]*observation[1]*observation[1]+diag_q[2]*observation[2]*observation[2]+diag_q[3]*observation[3]*observation[3]+diag_q[4]*observation[4]*observation[4]+diag_q[5]*observation[5]*observation[5]+diag_q[6]*observation[6]*observation[6]+diag_q[7]*observation[7]*observation[7]+diag_q[8]*observation[8]*observation[8]+diag_q[9]*observation[9]*observation[9]+diag_q[10]*observation[10]*observation[10]+diag_q[11]*observation[11]*observation[11]+diag_q[12]*observation[12]*observation[12]#observation[6]*observation[6]+observation[7]*observation[7]*observation[6]+10*observation[8]*observation[8]#a*np.exp(b*observation[8])+a*np.exp((-1)*b*observation[8])#cost_fwd+cost_ctrl
    ctrl_cost = r[0]*np.square(action[0])+r[1]*np.square(action[1])+r[2]*np.square(action[2])+r[3]*np.square(action[3])+r[4]*np.square(action[4])+r[5]*np.square(action[5])
...
def make_env(env_id, render_bool):
    ...
    if render_bool:
        ...
        env = gym.make('SoftLander-v0',render_mode = "human")
    else:
        env = gym.make('SoftLander-v0')
...
if __name__ == "__main__":
    ...
    for global_step in range(total_timesteps):
        ...
        if episode_t == episode_length :#or abs(next_obs[8])<0:
